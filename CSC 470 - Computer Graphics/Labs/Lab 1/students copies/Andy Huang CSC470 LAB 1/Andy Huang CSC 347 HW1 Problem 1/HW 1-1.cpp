...
void drawDino(GLfloat startX, GLfloat startY, int tilt, bool tilted, bool flip,int radius){
...
void myDisplay(void) // plot the sinc function, using world coordinates
{
	...
	/*
	glViewport(120, 440, 50, 30); // set the next viewport
	glRotatef(0, 120, 440, 0);
	drawPolyLineFile("Brontoi.dat"); // draw it again
	*/
	drawDino((screenWidth / 4) * 1, (screenHeight / 4) * 3, -90, 1, 0, 90); //Shows dinos in circle with heads at angles
	drawDino((screenWidth / 4) * 3, (screenHeight / 4) * 3 + 30, 0, 0, 0, 90);
	drawDino((screenWidth / 4) * 1, (screenHeight / 4) * 1, 270, 1, 1, 90);
	drawDino((screenWidth / 4) * 3, (screenHeight / 4) * 1 + 20, 0, 0, 1, 90);
}
